...
## Corpus Check
- 51 files · ~97,724 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 1264 nodes · 2600 edges · 72 communities (62 shown, 8 thin omitted)
- Extraction: 97% EXTRACTED · 3% INFERRED · 0% AMBIGUOUS · INFERRED: 68 edges (avg confidence: 0.85)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `01af23de`
...
6. `ExecutionAdapterTests` - 24 edges
...
Cohesion: 0.06
Nodes (30): build_pumpportal_exact_sell_payload(), build_pumpportal_lightning_sell_payload(), calculate_wallet_sell_percentage(), fetch_finalized_solana_transaction(), fetch_solana_signature_status(), finish_market_event_inbox_processing(), finish_market_event_inbox_validation(), normalize_solana_signature() (+22 more)
...
### Community 7 - "auth"
Cohesion: 0.09
Nodes (65): api_helius_webhook_sample(), auth(), can_retry_execution(), check_execution_timeout(), create_execution_order(), demo_can_retry(), demo_cannot_retry_risk(), demo_concurrent_idempotency() (+57 more)
...
### Community 25 - "consume_market_event_inbox_once"
Cohesion: 0.22
Nodes (10): claim_market_event_inbox_processing_batch(), consume_market_event_inbox_once(), mark_market_event_processed(), market_event_identity(), market_event_is_after_activation(), Identidad de un evento de mercado dentro de una transacción. La usan las dos…, Permite solo eventos recibidos y ocurridos desde la activación. `blockEventTs`…, Reserva filas validadas para un único consumidor con lease. (+2 more)
...
### Community 31 - "db"
Cohesion: 0.07
Nodes (45): api_helius_webhook_stats(), cleanup_finished_outcome_token(), complete_finished_signal_outcomes(), create_signal_outcome(), db(), establish_market_event_inbox_activation(), expire_old_signal_outcomes(), get_ambiguous_pumpportal_execution_orders() (+37 more)
...
### Community 34 - "app.py"
Cohesion: 0.07
Nodes (55): api_live_execution_readiness(), api_rpc_fallback_stats(), api_shadow_predictions(), api_shadow_stats(), api_training_checkpoint_freshness(), api_training_expired_preview(), api_training_stats(), api_training_stats_by_trader() (+47 more)
...
Cohesion: 0.20
Nodes (10): api_live_positions(), count_open_positions(), demo_liquidity_check(), demo_risk_mode_isolation(), demo_slippage_check(), get_daily_live_realized_pnl_sol(), get_live_position_summary(), risk_check() (+2 more)
...
Cohesion: 0.27
Nodes (10): _audit_model(), audit_shadow_models(), _fetch_json(), fetch_shadow_predictions(), main(), pair_completed_predictions(), _payoff(), _temporal_windows() (+2 more)
...
### Community 63 - "post"
Cohesion: 0.13
Nodes (21): demo(), demo_close_old(), demo_duplicate_check(), demo_exit_close(), demo_invalid_event(), demo_partial_close(), demo_partial_sell(), demo_profit() (+13 more)

### Community 64 - "route_market_event"
Cohesion: 0.20
Nodes (15): decide_live_position_exit(), evaluate_live_position_exit(), is_pumpportal_error_message(), market_event_index(), market_event_new_token_balance(), Índice de una operación dentro de su transacción, leído del evento. La…, Preserva la diferencia entre un saldo desconocido y un cero real. PumpPortal…, Apply an already deduplicated event using the existing live semantics.… (+7 more)
...
### Community 69 - "validated_block_event_ts"
Cohesion: 0.40
Nodes (6): market_event_block_ts(), market_event_from_inbox_row(), Timestamp on-chain validado: numérico, finito y positivo. ``None`` pasa como…, Momento on-chain del evento, leído de adentro del evento. Ausente significa…, Reconstruye el evento normalizado guardado en `market_event_inbox`. La fila…, validated_block_event_ts()

### Community 70 - "validate_market_event_inbox_once"
Cohesion: 0.33
Nodes (6): claim_market_event_inbox_validation_batch(), market_event_inbox_validation_worker(), Reserva eventos observados para validarlos sin ejecutar sus efectos. La reserva…, Valida un lote del inbox sin llamar al router ni aplicar efectos., Valida continuamente el inbox; nunca enruta eventos., validate_market_event_inbox_once()

### Community 71 - "record_helius_webhook_transactions"
Cohesion: 0.40
...
- **Why does `parse_watched_wallet_pump_events()` connect `parse_watched_wallet_pump_events` to `app.py`, `RpcFallbackBaselineTests`, `solana_rpc_fallback.py`, `record_helius_webhook_transactions`, `RpcFallbackPersistenceTests`, `pump_receipt`?**
  _High betweenness centrality (0.039) - this node is a cross-community bridge._
- **Why does `InboxRoundTripTests` connect `InboxRoundTripTests` to `pump_receipt`?**
