...
## Corpus Check
- 46 files · ~83,514 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 1029 nodes · 2135 edges · 62 communities (51 shown, 9 thin omitted)
- Extraction: 98% EXTRACTED · 2% INFERRED · 0% AMBIGUOUS · INFERRED: 47 edges (avg confidence: 0.85)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `7e44fa40`
...
8. `InboxRoundTripTests` - 19 edges
9. `main()` - 18 edges
...
### Community 25 - "auth"
Cohesion: 0.10
Nodes (56): auth(), demo(), demo_can_retry(), demo_cannot_retry_risk(), demo_close_old(), demo_concurrent_idempotency(), demo_daily_pnl(), demo_daily_pnl_isolation() (+48 more)
...
### Community 28 - "simulate_execution"
Cohesion: 0.15
Nodes (25): can_retry_execution(), check_execution_timeout(), create_execution_order(), create_execution_order_idempotent(), demo_controlled_retry(), demo_execution_failure(), demo_execution_timeout(), demo_idempotency_check() (+17 more)
...
### Community 34 - "app.py"
Cohesion: 0.07
Nodes (69): api_helius_webhook_sample(), api_helius_webhook_stats(), api_rpc_fallback_stats(), api_shadow_predictions(), api_training_checkpoint_freshness(), api_training_dataset(), api_training_dataset_preview(), api_training_expired_preview() (+61 more)
...
Cohesion: 0.27
Nodes (10): decide_live_position_exit(), evaluate_live_position_exit(), market_event_identity(), Apply an already deduplicated event using the existing live semantics.…, Guarda un punto de historial de market cap. Con firma, la escritura es…, Identidad de un evento de mercado dentro de una transacción. La usan las dos…, route_market_event(), save_token_history() (+2 more)
...
### Community 54 - "stream"
Cohesion: 0.17
Nodes (13): fetch_solana_balance_sol(), is_pumpportal_error_message(), mark_market_event_processed(), mark_signature_processed(), mark_stream_problem(), mark_stream_recovered(), post_discord_alert(), pumpportal_balance_monitor() (+5 more)
...
### Community 58 - "market_event_block_ts"
Cohesion: 0.22
Nodes (9): helius_webhook(), market_event_block_ts(), Registra lo que llegó por webhook. Observacional: no dispara nada. Devuelve…, Recibe transacciones de Helius. Solo mide; no alimenta decisiones., Timestamp on-chain validado: numérico, finito y positivo. ``None`` pasa como…, Momento on-chain del evento, leído de adentro del evento. Ausente significa…, record_helius_webhook_transactions(), validated_block_event_ts() (+1 more)

### Community 59 - "evaluate_buy"
Cohesion: 0.29
...
### Community 60 - "validate_market_event_inbox_once"
Cohesion: 0.25
...
### Community 61 - "market_event_index"
Cohesion: 0.50
Nodes (4): market_event_from_inbox_row(), market_event_index(), Índice de una operación dentro de su transacción, leído del evento. La…, Reconstruye el evento normalizado guardado en `market_event_inbox`. La fila…
...
- **Why does `parse_watched_wallet_pump_events()` connect `parse_watched_wallet_pump_events` to `app.py`, `market_event_block_ts`?**
